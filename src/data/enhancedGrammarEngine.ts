...
import { GrammarLevel, WordTile, ValidationResult, GrammarRules } from './types';
import { comprehensiveLevels45, getLevelById } from './comprehensiveLevels45';
import { timeExpressionValidator } from './timeExpressionSystem';
import { verbSelector } from './enhancedVerbDatabase';

export class EnhancedGrammarEngine {
  private timeValidator: typeof timeExpressionValidator;
  private verbSelector: typeof verbSelector;
  ...
  constructor() {
    this.timeValidator = timeExpressionValidator;
    this.verbSelector = verbSelector;
  }

  // Main validation function
  validateSentence(sentenceTiles: WordTile[], levelId: number): ValidationResult {
  ...
  private comprehensiveValidation(words: string[], level: GrammarLevel): { isCorrect: boolean; errors: string[]; suggestions: string[] } {
    const errors: string[] = [];
  ...
  private validateSentenceStructure(words: string[], level: GrammarLevel): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateVerbForms(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validatePresentTense(words: string[], verb: any, grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validatePastTense(words: string[], verb: any, grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    const mainVerb = this.findMainVerb(words);
    if (!mainVerb) return { isCorrect: true };
  ...
  private validatePerfectTense(words: string[], verb: any, grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    const mainVerb = this.findMainVerb(words);
  ...
  private validateContinuousTense(words: string[], verb: any, grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    const mainVerb = this.findMainVerb(words);
  ...
  private validateBaseForm(words: string[], verb: any, grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    const mainVerb = this.findMainVerb(words);
  ...
  private validateSubjectVerbAgreement(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateTimeExpressions(words: string[], grammarRules: GrammarRules, level: GrammarLevel): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    ...
    for (const timeExpr of timeExpressions) {
      ...
      if (!validation.isCorrect) {
        return {
          ...
          suggestions: [validation.suggestion].filter(Boolean) as string[]
        };
      }
    }
  ...
  private validateCriticalTenseComparison(timeExpressions: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
    for (const timeExpr of timeExpressions) {
  ...
  private validateAuxiliaryVerbs(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateQuestionStructure(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateYesNoQuestion(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateWhQuestion(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private validateNegativeStructure(words: string[], grammarRules: GrammarRules): { isCorrect: boolean; error?: string; suggestions?: string[] } {
  ...
  private findAuxiliary(words: string[]): string | undefined {
    const auxiliaries = ['am', 'is', 'are', 'was', 'were', 'have', 'has', 'do', 'does', 'did', 'will', 'would', 'can', 'could', 'should', 'must'];
    return words.find(word => auxiliaries.includes(word.toLowerCase()));
  }

  private hasSubject(words: string[]): boolean {
    return this.findSubject(words) !== undefined;
  }

  private hasVerb(words: string[]): boolean {
    return this.findMainVerb(words) !== undefined;
  }

  private hasObject(words: string[]): boolean {
  ...
  private isThirdPersonSingular(subject: string | undefined): boolean {
    return ['he', 'she', 'it'].includes(subject?.toLowerCase() || '');
  }

  private getExpectedAuxiliary(subject: string, auxiliaryType: string): string {
    const patterns = this.auxiliaryPatterns[auxiliaryType as keyof typeof this.auxiliaryPatterns];
    if (!patterns) return auxiliaryType;

    const subjectKey = subject.toLowerCase();
    return patterns[subjectKey as keyof typeof patterns] || auxiliaryType;
  }

  private extractTimeExpressions(sentence: string): string[] {
    const timeWords = [
  ...
  // Feedback generation
  private generatePositiveFeedback(level: GrammarLevel, sentence: string): string {
  ...
  private generateCorrectiveFeedback(errors: string[], level: GrammarLevel): string {
