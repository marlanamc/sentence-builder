...
// Clean explanation strings for all levels
...
// Apply all clean explanations
Object.entries(cleanExplanations).forEach(([levelId, cleanExplanation]) => {
  const pattern = new RegExp(`(id:\\s*${levelId},[\\s\\S]*?explanation:\\s*['"\`])([^'"\`]+)(['"\`])`, 'g');
  content = content.replace(pattern, `$1${cleanExplanation}$3`);
});

// Write updated content
fs.writeFileSync(levelsPath, content, 'utf8');
console.log('✅ Rebuilt all explanation strings with clean formatting');
